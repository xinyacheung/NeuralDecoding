...
def load_dataloader(config):
    ...
    test_ds = SpikeDataset(config["neuron_ranges"], config["stimuli_list"], categ='validation', seed = config["seed"] )
...
def train_model(config):
    ...
    model = VideoUNet((3,32,32),config["num_neurons"],32).to(device)
